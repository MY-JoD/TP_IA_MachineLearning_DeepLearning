...
void learnerMLR() {
  ...
  do {
    ...
    for (int i = 0; i < 24; i++)
    {
      tmpSomme0 +=  ( hypRLin(xi[i]) - yi[i] );
      tmpSomme1 +=  (hypRLin(xi[i]) - yi[i])*xi[i];
    }
    ...
    for (int i = 0; i < 24; i++)
    {
      erreur +=  ( hypRLin(xi[i]) - (yi[i]) )*( hypRLin(xi[i]) - (yi[i]) );
    }
    erreur = erreur/48.;
...
// Fonction de prédiction duty cycle => vitesse
double predictVitesse(double x) {
  return hypRLin(x);
}

// Fonction de prédiction vitesse => duty cycle
